...
type FormData = {
  nome: string; telefone: string; email: string; horario: string; observacoes: string
}

export default function Home() {
  const [vagas, setVagas] = useState<Vagas | null>(null)
  const [form, setForm] = useState<FormData>({ nome: '', telefone: '', email: '', horario: '', observacoes: '' })
  ...
  return (
    <main>
      ...
      <section style={{ background: 'linear-gradient(160deg,#1A0A0A 0%,#3A1010 50%,#1A0A0A 100%)', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '80px 24px 60px', position: 'relative' }}>
        ...
        <Image src="/logo.png" alt="Neon Pizzaria" width={260} height={130} style={{ objectFit: 'contain', marginBottom: 24, filter: 'drop-shadow(0 0 24px rgba(201,168,76,0.5))' }} priority />
        ...
        <div style={{ fontFamily:"'Cormorant Garamond',serif", fontStyle:'italic', fontSize:15, color:'rgba(255,255,255,0.4)', marginBottom:48 }}>1 entrada · 1 prato principal · 1 sobremesa</div>
      ...
      <section style={{ background:'#F5EDD8', padding:'80px 24px' }}>
        <div style={{ maxWidth:680, margin:'0 auto' }}>
          ...
          <MenuCat title="Entradas" items={[{name:'Bruschetta Italiana Tradizionale',desc:'Molho confit e molho pesto'},{name:'Arancini di Formaggio',desc:'Clássicos bolinhos de risoto com queijo'}]} />
          <MenuCat title="Prato Principal" items={[{name:'Pizza Napoletana',desc:'4 fatias · sabores do nosso cardápio'},{name:'Risotto ai Funghi e Tartufo',desc:'Risoto cremoso de cogumelos com toque de azeite trufado'},{name:'Risotto ai 3 Formaggi',desc:'Risoto cremoso com uma seleção de três queijos'},{name:'Massa Carbonara',desc:'Massa al dente com molho cremoso de ovos, queijo pecorino e guanciale'}]} />
          <MenuCat title="Sobremesas" items={[{name:'Petit Gâteau com Gelato',desc:'Bolo de chocolate cremoso com sorvete de creme'},{name:'Tiramissu',desc:'Clássica sobremesa italiana com camadas de biscoito embebido em café e creme de mascarpone'},{name:'Brownie com Nutella e Sorvete',desc:'Brownie servido com Nutella e sorvete de creme'}]} />
      ...
      <section style={{ background:'#EDE0C4', padding:'60px 24px' }}>
        <div style={{ maxWidth:640, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginTop:32 }}>
            {[{icon:'💌',text:'Cartão romântico personalizado com foto para os 20 primeiros casais'},{icon:'🕯️',text:'Decoração temática com velas em toda a experiência'},{icon:'🌹',text:'Mesa decorada com flores para casais com reserva'},{icon:'🎵',text:'Playlist especial selecionada para a noite dos namorados'}].map((item,i) => (
        ...
        <div style={{ maxWidth:560, margin:'0 auto' }}>
          {!sucesso ? (
            <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(201,168,76,0.3)', borderRadius:12, padding:'48px 40px' }}>
              ...
              <FField label="Nome completo"><input style={iStyle} placeholder="Seu nome completo" value={form.nome} onChange={e=>setForm(f=>({...f,nome:e.target.value}))} /></FField>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                <FField label="Telefone / WhatsApp"><input style={iStyle} placeholder="(12) 99999-9999" value={form.telefone} onChange={e=>setForm(f=>({...f,telefone:e.target.value}))} /></FField>
                <FField label="E-mail"><input style={iStyle} type="email" placeholder="seu@email.com" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} /></FField>
              </div>
              ...
              <FField label="Observações especiais (opcional)"><input style={iStyle} placeholder="Ex: aniversário, pedido de casamento..." value={form.observacoes} onChange={e=>setForm(f=>({...f,observacoes:e.target.value}))} /></FField>

              <hr style={{ border:'none', borderTop:'1px solid rgba(201,168,76,0.15)', margin:'24px 0' }} />

              <div style={{ background:'rgba(201,168,76,0.08)', borderRadius:8, padding:20, marginBottom:24, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div>
                  ...
                  <div style={{ fontSize:11, color:'rgba(255,255,255,0.3)' }}>Desconta integralmente no consumo</div>
                </div>
                <div style={{ textAlign:'right', fontSize:11, color:'rgba(255,255,255,0.3)', lineHeight:1.7 }}>R$ 139 × 2 pessoas<br/>Pago via InfinitPay</div>
              </div>
              ...
              <p style={{ fontSize:11, color:'rgba(255,255,255,0.3)', textAlign:'center', marginTop:16, lineHeight:1.6 }}>Cancelamentos com reembolso total até 48h antes do evento.<br/>Após o pagamento você receberá a confirmação por WhatsApp.</p>
            </div>
          ) : (
            <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(201,168,76,0.3)', borderRadius:12, padding:'48px 40px', textAlign:'center' }}>
              ...
              <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:17, color:'rgba(255,255,255,0.7)', lineHeight:1.6, marginBottom:20 }}>Sua mesa está garantida. Finalize o pagamento para confirmar.</div>
              <div style={{ fontFamily:"'Cinzel',serif", fontSize:32, color:'#fff', margin:'20px 0' }}>{sucesso.codigo}</div>
              <div style={{ fontSize:13, color:'rgba(255,255,255,0.4)', marginBottom:24 }}>Guarde este código. Apresente na entrada.</div>
              ...
              <p style={{ fontSize:11, color:'rgba(255,255,255,0.3)', marginTop:16, lineHeight:1.6 }}>Após o pagamento, nossa equipe confirmará sua reserva pelo WhatsApp em até 2h.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <footer style={{ background:'#0D0808', padding:'32px 24px', textAlign:'center' }}>
        <Image src="/logo.png" alt="Neon Pizzaria" width={100} height={50} style={{ objectFit:'contain', opacity:0.6 }} />
...
const iStyle: React.CSSProperties = { width:'100%', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(201,168,76,0.25)', borderRadius:6, padding:'13px 16px', color:'#fff', fontFamily:'Lato,sans-serif', fontSize:14, outline:'none' }
